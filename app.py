...
DISEASE_INFO = {
    'Healthy_leaves': {
        'status': 'info',
        'message': "✨ Your plant is looking great! Maintain regular watering and sunlight.",
        'steps': ["No intervention needed.", "Continue routine monitoring."]
    },
    'Anthracnose lesions': {
        'status': 'error',
        'message': "🚨 Fungal infection detected (Colletotrichum orbiculare).",
        'steps': [
    ...
    'Downy Mildew': {
        'status': 'error',
        'message': "🚨 Severe fungal threat. Spreads rapidly in high humidity.",
        'steps': [
            "Increase plant spacing to improve air circulation.",
            "Apply targeted fungicides like Mancozeb or Ridomil Gold.",
            "Plant resistant varieties in the next cycle."
        ]
    },
    'Leaf Miner': {
        'status': 'warning',
        'message': "⚠️ Insect pest detected. Larvae are tunneling inside the leaves.",
        'steps': [
            "Use yellow sticky traps to catch adult flies.",
            "Introduce natural predators like parasitic wasps (Diglyphus isaea).",
            "Apply Neem oil or Spinosad-based organic insecticides."
        ]
    },
    'Powdery_mildew': {
        'status': 'error',
        'message': "🚨 White fungal coating detected on leaf surfaces.",
        'steps': [
            "Apply sulfur-based fungicides or Potassium Bicarbonate.",
            "Prune overcrowded areas to reduce humidity.",
            "Ensure the plant receives adequate morning sunlight."
        ]
    }
}
...
def get_base_model(model_name, num_classes):
    # Use .startswith or a mapping to ensure Hybrid keys don't break this if called
...
# Inside HybridModel forward:
def forward(self, x):
    feat_a = self.model_a(x)
    feat_b = self.model_b(x)
    
    # Handle InceptionV3 returning a tuple/namedtuple
    if isinstance(feat_a, torch.Tensor): out1 = torch.flatten(feat_a, 1)
    else: out1 = torch.flatten(feat_a.logits if hasattr(feat_a, 'logits') else feat_a[0], 1)
        
    if isinstance(feat_b, torch.Tensor): out2 = torch.flatten(feat_b, 1)
    else: out2 = torch.flatten(feat_b.logits if hasattr(feat_b, 'logits') else feat_b[0], 1)
        ...
        if st.button("🚀 Analyze Leaf"):
            with st.spinner('Initializing Neural Networks...'):
                ...
                if model:
                    ...
                    #    if label == 'Healthy_leaves':
                    #        st.info("Plant is healthy. No action required.")
                    #    elif label == 'Anthracnose lesions':
                    #        st.write("• **Action:** Remove infected leaves. Apply copper-based fungicide.") # ... and so on
                    ...
                    #    elif label == 'Leaf Miner':
                    #        st.write("• **Action:** Rotate crops. Apply protectant fungicides.")
                        # Match these exactly to your TARGET_CLASSES list
                    with st.expander("📋 View Action Plan", expanded=True):
                        # Fetch data for the detected label
                        ...
                        if info:
                            ...
                            # 2. Display Action Steps
                            st.markdown("### Recommended Steps:")
                            for step in info['steps']:
                                st.write(f"• {step}")
                        else:
                            st.warning("No specific action plan found for this condition.")
